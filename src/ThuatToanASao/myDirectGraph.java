...
public class myDirectGraph {
    ...
    private int[][] edgeValues;   // Giá trị của mỗi cạnh
...
//    }
    ...
    public void addEdge(int v, int w, int value) {
        ...
        if (E >= edgeValues.length) {
            // Nếu số lượng cạnh đã vượt quá kích thước của mảng edgeValues, mở rộng mảng
            int newSize = edgeValues.length * 2; // Có thể điều chỉnh kích thước tùy ý
            edgeValues = Arrays.copyOf(edgeValues, newSize);
        }
        E++;
        adj[v].add(w);
//        adj[w].add(v);
        edgeValues[v][w] = value;
    }
    
    public void addEdge(String v, String w, int value) {
     ...
     // In ra đồ thị theo định dạng yêu cầu
        for (int v = 0; v < V; v++) {
            ...
            for (int w : G.adj(v)) {
                ...
                System.out.print(G.getEdgeValue(v, w) + " ");
                System.out.print("\n");
            }
            System.out.println();
        }
